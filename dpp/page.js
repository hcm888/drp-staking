...
export default function StakeDapp() {
  ...
  return (
    <div style={containerStyle}>
      ...
      {账户 && 合同 && (
        <div 样式={funcAreaStyle}>
          <div 样式={infoStyle}>
            <p>钱包余额：<span 样式={boldText}>{balance} 代币</span></p>
          </div>

          <div 样式={stakeAreaStyle}>
            <h3>质押操作</h3>
            <input
              type="number"
              ...
              disabled={loading}
            />
            <button onClick={handleStake} 样式={btnStyle} disabled={loading || !stakeAmount}>
              {loading ? '处理中...' : '确认质押'}
            </button>
          </div>

          <div 样式={recordAreaStyle}>
            <h3>我的质押记录（点击赎回）</h3>
            {stakeRecords.length === 0 ? (
              <p>暂无质押记录</p>
            ) : (
              <div 样式={recordListStyle}>
                {stakeRecords.map((item) => (
                  <div key={item.index} 样式={recordItemStyle}>
                    <p>档位{item.index+1} | 金额：{item.amount}代币 | 状态：{item.status} | 时间：{item.stakeTime}</p>
                    {item.status === '已质押' && (
                      <button onClick={() => handleUnstake(item.index)} 样式={unstakeBtnStyle} disabled={loading}>赎回</button>
                    )}
                  </div>
                ))}
              </div>
...
// 手机适配样式（不用改）
常量 containerStyle = { 内边距: '1.5rem', maxWidth: '700px', 边距: '0 auto', 字体系列: 'Arial' }
常量 titleStyle = { textAlign: 'center', color: '#0070f3', marginBottom: '1rem' }
常量 tipStyle = { textAlign: 'center', color: '#666', fontSize: '0.9rem' }
常量btnStyle = { 宽度: '100%', 填充: '1rem', backgroundColor: '#0070f3', color: 'white', 边框: 'none', 圆角: '8px', 字体大小: '1rem', 光标: 指针, 上边距: '1rem' }
常量 accountStyle = { textAlign: 'center', padding: '1rem', backgroundColor: '#f5f5f5', borderRadius: '8px', marginTop: '1rem' }
常量 funcAreaStyle = { marginTop: '2rem', gap: '1.5rem', display: 'flex', flexDirection: 'column' }
常量 infoStyle = { padding: '1rem', backgroundColor: '#f0f8ff', borderRadius: '8px' }
常量 boldText = { fontWeight: 'bold', color: '#0070f3' }
常量 stakeAreaStyle = { padding: '1rem', border: '1px solid #eee', borderRadius: '8px' }
常量 inputStyle = { width: '100%', padding: '0.8rem', margin: '1rem 0', border: '1px solid #ccc', borderRadius: '4px', fontSize: '1rem' }
常量 recordAreaStyle = { padding: '1rem', border: '1px solid #eee', borderRadius: '8px' }
常量 recordListStyle = { gap: '1rem', display: 'flex', flexDirection: 'column', marginTop: '1rem' }
常量 recordItemStyle = { padding: '1rem', backgroundColor: '#f9f9f9', borderRadius: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }
常量 unstakeBtnStyle = { 填充: '0.5rem 1rem', 背景颜色: '#ff4d4f', 颜色: '白色', 边框: 'none', 圆角: '4px', 光标: '指针' }
